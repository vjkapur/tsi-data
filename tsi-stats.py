...
for target_attr in date_attrs:
    tsi_311_data[target_attr] = tsi_311_data.ADDDATE.astype(str).str.slice(stop=date_attrs[target_attr])
...
tsi_311_data.ADDDATE.astype(str).str.slice(stop=10).value_counts().sort_index().rolling(7).sum().to_csv('tsi_counts_by_7day.csv', header=True)